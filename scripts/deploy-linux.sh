...
id koralevents2 >/dev/null 2>&1 || useradd --system --home "$root/shared" --shell /usr/sbin/nologin koralevents2
chgrp koralevents2 "$root" "$root/releases" "$root/shared"
chmod 750 "$root" "$root/releases" "$root/shared"
chown -R koralevents2:koralevents2 "$root/shared/data" "$root/shared/uploads" "$root/backups"
...
cat > /etc/systemd/system/koralevents2.service <<'UNIT'
[Unit]
...
User=koralevents2
Group=koralevents2
WorkingDirectory=/opt/koralevents2/current
EnvironmentFile=/opt/koralevents2/shared/app.env
EnvironmentFile=/opt/koralevents2/current/release.env
Environment=NODE_ENV=production HOSTNAME=127.0.0.1 PORT=3111 NEXT_TELEMETRY_DISABLED=1
...
ReadWritePaths=/opt/koralevents2/shared /opt/koralevents2/current/.next/cache
UMask=0077
[Install]
WantedBy=multi-user.target
UNIT
mkdir -p "$release/.next/cache"
chown -R koralevents2:koralevents2 "$release/.next/cache"
ln -sfn "$release" "$root/current.new"
mv -Tf "$root/current.new" "$root/current"
systemctl daemon-reload
systemctl enable koralevents2 >/dev/null
rollback() {
    echo 'New release failed. Restoring previous application version.' >&2
    if [[ -n "$previous" && -d "$previous" ]]; then
        ln -sfn "$previous" "$root/current.new"
        mv -Tf "$root/current.new" "$root/current"
        systemctl restart koralevents2
    else
        systemctl stop koralevents2
    fi
